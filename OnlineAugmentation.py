...
class ImageDataGenerator(object):
    ...
    def get_random_transform(self, img_shape, seed=None):
        ...
        # Custom Features
        blur = (np.random.random() < 0.5) * self.blurring
        white_to_color = None
        if self.white_to_color:
            white_to_color = np.random.randint(0, 255, size = 3, dtype = 'uint8')
    ...
    def apply_transform(self, x, transform_parameters):
        ...
        # Custom Features
        if transform_parameters.get('blur', False):
            x = scipy.ndimage.gaussian_filter(x, sigma = 3)

        if transform_parameters.get('white_to_color') is not None:# TODO make this work if channel axis is different
            count = np.zeros((x.shape[img_col_axis], x.shape[img_row_axis]), dtype = 'uint8')
            count[x[:,:,0] > 200] += 1
            count[x[:,:,1] > 200] += 1
            count[x[:,:,2] > 200] += 1
            x[count == 3] = transform_parameters['white_to_color']
